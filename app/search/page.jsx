...
export default function SearchPage() {
  ...
  const q = (params.get("e") || params.get("q") || "").toLowerCase();

  const results = useMemo(() => {
    if (!q) return [];
    return trolls.filter(t => {
      const hay = (t.name + " " + t.synopsis + " " + t.region.join(" ") + " " + t.habitat.join(" ")).toLowerCase();
  ...
  return (
    <div className="space-y-4">
      ...
      {q ? <p className="text-sm text-gray-600">Results for <strong>{q}</strong></p> : <p className="text-sm">Type something in the search box on the home page.</p>}
        ...
        {results.map(t => (
          <Link key={t.slug} href={`/catalog/${t.slug}`} className="card block hover:shadow-lg transition">
            <div className="aspect-[16/9] bg-gray-100 rounded-xl mb-3 flex items-center justify-center overflow-hidden">
              <img src={t.name && t.image ? t.image : "/images/placeholder.svg"} alt={t.name} className="w-full h-full object-cover" />
            </div>
